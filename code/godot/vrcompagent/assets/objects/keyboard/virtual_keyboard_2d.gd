...
func is_xr_class(class_name_str : String) -> bool:
	return class_name_str == "XRToolsVirtualKeyboard2D"
...
func on_key_pressed(scan_code_text: String, unicode: int, shift: bool):
	...
	if scan_code == 0:
		var t := scan_code_text.to_lower()
		if t == "enter" or t == "return" or t == "ui_accept":
			scan_code = KEY_ENTER
		elif t == "backspace" or t == "bksp" or t == "ui_text_backspace":
	...
	# Also send a release event so UI widgets that expect press+release work reliably.
	var input_up := InputEventKey.new()
	input_up.physical_keycode = scan_code
	input_up.unicode = unicode if unicode else scan_code
	input_up.pressed = false
	input_up.keycode = scan_code
	input_up.shift_pressed = shift
	Input.parse_input_event(input_up)
